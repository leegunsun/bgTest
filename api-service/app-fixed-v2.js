...
// Fixed version for bind-mounted files - writes directly instead of atomic rename
async function switchEnvironment(targetEnv) {
    try {
        // Read current file content for backup
        let backupContent = '';
        try {
            backupContent = await fs.readFile(ACTIVE_ENV_FILE, 'utf8');
        } catch (error) {
            console.log('Could not read current file for backup, proceeding anyway');
        }
        
        // Create new configuration content
        const newContent = `set $active "${targetEnv}";`;
        
        // Write directly to file (bind-mount friendly approach)
        await fs.writeFile(ACTIVE_ENV_FILE, newContent + '\n', 'utf8');
        ...
        try {
            ...
            const verifyActive = await getCurrentActiveEnvironment();
            if (verifyActive !== targetEnv) {
                throw new Error(`Switch verification failed: expected ${targetEnv}, got ${verifyActive}`);
            }
            console.log(`Switch verification successful: now serving ${targetEnv}`);
            
        } catch (nginxError) {
            console.error(`NGINX operation failed: ${nginxError.message}`);
            
            // Try to rollback by restoring original content
            if (backupContent) {
                try {
                    await fs.writeFile(ACTIVE_ENV_FILE, backupContent, 'utf8');
                    console.log('Rolled back configuration to previous state');
                } catch (rollbackError) {
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🔧 Using direct file write (bind-mount compatible)`);
});